...
<%
	...
	if (!(partnerApi.getPartnerId() != null && partnerApi.getPartnerId().length() > 0)) {
		out.print("{\"status\":\"error\",\"result\":4001,\"msg\":\"partnerId is not exist!\"}");
		return;
	}
	partnerInfo = PartnerService.getInstance().getNameLoadingCache(partnerApi.getPartnerId());
	if (partnerInfo == null) {
		out.print("{\"status\":\"error\",\"result\":4001,\"msg\":\"partnerId is not exist!\"}");
		return;
	}
	if (partnerInfo != null && partnerInfo.getKeyAES() != null && partnerInfo.getKeyAES().length() > 0) {
